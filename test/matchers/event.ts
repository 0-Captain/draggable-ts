...
// function toHaveBeenCalledWithEvent(jestFunction: { mock: any; }, expectedEventConstructor: { type: any; name: any; }) {
...
function toHaveBeenCalledWithEventProperties(
  jestFunction: { mock: any },
  expectedProperties: { [s: string]: unknown } | ArrayLike<unknown>
) {
  const mockFunction = jestFunction.mock;
  const mockCalls = mockFunction.calls;
  const event = mockCalls[0][0];
